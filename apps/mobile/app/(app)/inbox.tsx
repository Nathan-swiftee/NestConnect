...
import { listTime, useConversations, useLogout, useSession, useViews } from "@ding/client";
    ...
    >
      ...
      <View className="flex-1 gap-1">
        <View className="flex-row items-baseline gap-2">
          <Text
            numberOfLines={1}
            className={`flex-1 text-lg ${unread ? "font-semibold text-fg" : "font-medium text-fg"}`}
          >
        ...
        <View className="flex-row items-center gap-2">
          ...
          {conv.slaDueAt && new Date(conv.slaDueAt).getTime() < Date.now() ? (
            <Text className="text-2xs font-medium text-danger">Overdue</Text>
          ) : null}
        </View>
      </View>
    </Pressable>
  ...
  return (
    <View style={{ backgroundColor: c.bg, paddingTop: insets.top }} className="flex-1">
      ...
      <FlatList
        ...
        // A shared inbox is read by pulling — this is the gesture people expect,
        // and it also runs the on-demand Gmail fetch behind `useRefresh` on web.
        refreshControl={
          <RefreshControl refreshing={list.isRefetching && !list.isFetchingNextPage} onRefresh={() => list.refetch()} tintColor={c.brand} />
        }
        ...
        ListFooterComponent={
          list.isFetchingNextPage ? <ActivityIndicator color={c.brand} className="py-4" /> : null
        }
        ListEmptyComponent={
          list.isLoading ? (
            ...
            <View className="items-center px-8 py-16">
              <Text className="text-lg font-medium text-fg">Nothing here</Text>
