...
```mermaid
gantt
    title Nest Connect delivery roadmap (indicative)
...
**Exit:** Swiftee handles live WhatsApp customer chats in Nest Connect on one number, end-to-end.
...
**Exit:** a shared `support@` inbox is handled in Nest Connect with the same routing/collaboration as WhatsApp.
...
**Exit:** Swiftee runs official WhatsApp client groups inside Nest Connect.
...
2. **Routing before groups** — the team-inbox mechanics are what make Nest Connect valuable; groups build on them.
